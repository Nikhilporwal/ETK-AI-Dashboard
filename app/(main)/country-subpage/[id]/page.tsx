                ...
                <div className="max-w-[1400px] mx-auto w-full flex flex-col md:flex-row justify-between gap-8">
                    ...
                    <div className="flex-1 space-y-6">
                        <div className="flex items-center gap-4">
                            ...
                            <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">{countryName}</h1>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                            {['Share', 'Compare countries', 'Add to watchlist', 'Set alert', 'Speak to expert'].map(btn => (
                                <button key={btn} className="px-4 py-2 border border-slate-200 rounded-lg bg-white text-xs font-semibold text-slate-600 hover:bg-slate-50">
                                    {btn}
                                </button>
                            ))}
                        </div>
            ...
            <main className="max-w-[1400px] mx-auto space-y-10 mt-10 px-4">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {result?.context?.data?.map((arr: any, idx: number) => (
                            <div key={idx} className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4">
                                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider h-8">
                                    {arr?.[0] || "Indicator"}
                                </h3>
                                <div className="text-2xl font-bold">{arr?.[1] || 0}/{arr?.[2] || 100}</div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-4 font-semibold">
                        <h2 className="text-lg font-semibold ml-1">Opportunities & Demand</h2>
