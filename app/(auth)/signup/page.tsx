  ...
  return (
    <form onSubmit={handleSignup} className="w-full max-w-[500px] mx-auto space-y-8">
      <div className="space-y-6">
        ...
        {/* Strength */}
        <div className="bg-white border border-[#A5B4FC]/30 rounded-2xl p-6 space-y-4 shadow-sm">
          <span className={cn("text-sm font-medium", getStrengthColorClass())}>
            {getStrengthLabel()}
          </span>
          ...
          <ul className="space-y-2">
            {requirements.map((req, idx) => {
              const isPassed = req.test(password);
              return (
                <li key={idx} className={cn("flex items-center gap-2 text-xs", isPassed ? "text-[#0EA497]" : "text-[#6B7280]")}>
                  <Check className={cn("w-3 h-3", isPassed ? "text-[#0EA497]" : "text-slate-300")} />
                  {req.label}
                </li>
              );
            })}
      ...
      <div className="space-y-6 text-center">
        <Button
          ...
          variant="primary"
          className="w-full h-14 text-base font-bold"
          disabled={passedCount < 4 || loading}
        >
          {loading ? (
            <Loader variant="full" message="Creating account..." />
          ) : (
        ...
        <p className="text-xs text-slate-500">
          By continuing, you agree to our{" "}
          <Link href="/terms" className="text-[#0EA497] underline">
            Terms
          </Link>{" "}
          and{" "}
          <Link href="/privacy" className="text-[#0EA497] underline">
            Privacy Policy
          </Link>
        </p>
      </div>
