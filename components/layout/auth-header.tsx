...
export function AuthHeader() {
    ...
    const { step: currentStep = 0, total: totalSteps = 4 } = stepConfigs[pathname] || {}
            ...
            <div className="max-w-[1440px] mx-auto px-6 h-[80px] flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1 text-sm">
                    <span className="text-slate-500">
                        {["/login", "/forgot-password", "/verify-otp", "/reset-password"].includes(pathname) ? "Don't have an account?" : "Already have an account?"}
                    </span>
                    <Link
                        href={pathname === "/login" ? "/" : "/login"}
                        className="text-[#0EA497] font-bold underline"
                    >
                        {pathname === "/login" ? "Get Started" : "Login"}
                    </Link>
                </div>
            </div>
